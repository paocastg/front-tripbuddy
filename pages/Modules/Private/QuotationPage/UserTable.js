...
const UserTable = props => (
  ...
  <Table style = {{ width: '1400px' }}>
    ...
    <tbody style={{ background: '#2C2C2C', fontFamily: 'Geomanist-Regular', color: '#fff', cursor: 'pointer', border: '2px solid' }}>
      {
        props.users && props.users.map((user, key) => (
          <tr key={key} style={{ border: ' 0.5px solid' }}>
            ...
            <td><button
            style={{ background: '#2C2C2C', fontFamily: 'Geomanist-Regular', color: '#fff', cursor: 'pointer' }} className="button muted-button" onClick={() => { props.editRow(user) }}>{ user.cotizaciones.length + ' ' }
            </button>
            </td>
          </tr>
        ))
