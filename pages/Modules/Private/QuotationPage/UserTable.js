...
const UserTable = props => (
  ...
  <center>
  <Table>
    <thead style={{ border: '#00B2E3 0.5px solid' }}>
      <tr>
                      <th>Destinos</th>
                      <th>Descripcion</th>
                      <th>Categoria</th>
    ...
    <tbody>
      {
        props.users && props.users.map((user, key) => (
          <tr style={{ border: 'rgb(250, 244, 244) 15.5px solid', height: '60px' }} key={key} onClick={() => { props.editRow(user) }}>
            ...
            <td>{ user.categoria && user.categoria.map((categorias, i) => <ul><li>{categorias.nombre.split('/([!,?,.])/') + '  ' }</li></ul>)}</td>
